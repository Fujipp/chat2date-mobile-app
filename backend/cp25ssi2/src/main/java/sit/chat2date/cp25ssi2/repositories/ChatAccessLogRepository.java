...
public interface ChatAccessLogRepository extends JpaRepository<ChatAccessLog, String> {

    @Query("SELECT c FROM ChatAccessLog c WHERE c.roomId = :roomId AND c.userId = :userId ORDER BY c.createdAt DESC LIMIT 1")
    Optional<ChatAccessLog> findLatestByRoomIdAndUserId(@Param("roomId") String roomId, @Param("userId") String userId);
    ...
    List<ChatAccessLog> findLatestStatusByRoomId(@Param("roomId") String roomId);

    boolean existsByRoomIdAndUserIdAndActionType(String roomId, String userId, ChatAccessActionType actionType);
}
