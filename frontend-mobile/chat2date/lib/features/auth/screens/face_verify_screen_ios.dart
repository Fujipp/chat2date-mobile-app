// lib/screens/auth/face_verify_screen_ios.dart
import 'dart:async';
import 'dart:convert';
import 'dart:io';
...
class FaceVerifyScreenIos extends ConsumerStatefulWidget {
  const FaceVerifyScreenIos({super.key});

  @override
  ConsumerState<FaceVerifyScreenIos> createState() =>
      _FaceVerifyScreenIosState();
}

class _FaceVerifyScreenIosState extends ConsumerState<FaceVerifyScreenIos>
  ...
  Future<void> _initCamera() async {
    try {
      ...
      debugPrint('❌ initCamera (iOS) error: $e\n$s');
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _onFrame(CameraImage img) async {
    ...
    try {
      ...
      debugPrint('❌ _onFrame (iOS) error: $e\n$s');
    } finally {
      _processingFrame = false;
    }
  }
  ...
  Future<Uint8List?> _captureSelfieBytes() async {
    try {
      ...
      debugPrint('❌ _captureSelfieBytes (iOS) error: $e\n$s');
      return null;
    }
  }
  ...
  Future<void> _goLoading() async {
    ...
    try {
      ...
      debugPrint('❌ stopImageStream (iOS) error: $e\n$s');
    }
    ...
    try {
      ...
      if (livenessPass && selfieBytes64 != null && idFaceBase64 != null) {
        ...
        debugPrint(
          '[KYC iOS] RESULT from BE: match=$matched, score=$score, raw=$vr',
        );
      } else {
        debugPrint(
          '[KYC iOS] SKIP verify (missing selfieBytes or idFaceBase64)',
        );
      }
      ...
      debugPrint('❌ _goLoading (iOS) error: $e\n$s');
      ...
      body: Stack(
        children: [
          ...
          const Positioned.fill(
            child: IgnorePointer(
              child: CustomPaint(
                painter: _FaceScanMaskPainterIos(holeDiameter: 236),
              ),
            ),
          ),
          Center(
            child: SizedBox(
              ...
              height: ringSize,
              child: CustomPaint(
                painter: _FaceScanRingPainterIos(
                  progress: _progress,
                  tickCount: 72,
                ),
              ),
...
class _FaceScanRingPainterIos extends CustomPainter {
  final double progress;
  final int tickCount;

  _FaceScanRingPainterIos({required this.progress, this.tickCount = 72});
  ...
  @override
  bool shouldRepaint(covariant _FaceScanRingPainterIos old) =>
      old.progress != progress || old.tickCount != tickCount;
}

class _FaceScanMaskPainterIos extends CustomPainter {
  final double holeDiameter;

  const _FaceScanMaskPainterIos({required this.holeDiameter});
  ...
  @override
  bool shouldRepaint(covariant _FaceScanMaskPainterIos oldDelegate) =>
